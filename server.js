...
// 🀄 ৫-রিলস ওরিজিনাল মাহজং টাইলস ট্র্যাডিশনাল ক্যাসিনো র্যান্ডমাইজেশন প্রতীক তালিকা ভাই ভাই
const mahjongTilesPool = ["GREEN_DRAGON", "RED_DRAGON", "WHITE_DRAGON", "ONE_BAMBOO", "NINE_DOTS", "BARROW", "FA_FA"];

// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে
app.get('/api/mahjong-balance', async (req, res) => {
...
// 🛫 ২. মাহজং ওয়ে ৫-রিল কোর স্লট体验 স্পিন রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/mahjong-spin', async (req, res) => {
    ...
    try {
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.mahjong_target) ? balResponse.data.mahjong_target : null;

        let spinResults, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP ও ৫-রিলস মাহজং গাণিতিক লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            loopSafety++;
            
            // ৫টি মাহজং রিলের র্যান্ডম স্টপ টাইলস প্রতীক জেনারেটর ভাই ভাই
            spinResults = [];
            for (let r = 0; r < 5; r++) {
                spinResults.push(mahjongTilesPool[Math.floor(Math.random() * mahjongTilesPool.length)]);
            }

            // ওরিজিনাল মাহজং স্লট পেলাইন ট্র্যাকিং মেথড: রিলসের ফ্রিকোয়েন্সি ম্যাচ কাউন্টার
            let countsMap = {};
            ...
            if (maxSameSymbolsCount >= 3) {
                finalStatus = "win";
                // 🚀 [ডাইনামিক কম্বো মাল্টিপ্লায়ার ওডস]: ৩টি মিললে ৩ গুণ, ৪টি মিললে ১৫ গুণ, ৫টি মিললে ১০০ গুণের গ্র্যান্ড জ্যাকপট!
                if (maxSameSymbolsCount === 3) winMultiplier = 3.00;
                else if (maxSameSymbolsCount === 4) winMultiplier = 15.00;
                else winMultiplier = 100.00; 
            } else {
                finalStatus = "lose";
                winMultiplier = 0.00;
            }
            ...
            if (adminTriggeredPrize) {
                ...
                if (adminTriggeredPrize === "force_jackpot" && winMultiplier === 100.00) isLoopActive = false;
            } else {
                // ১০০ গুণের মাহজং গ্র্যান্ড জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে কড়া সুরক্ষায় টাইট ০.৫% এ লক ভাই ভাই
                if (winMultiplier === 100.00 && Math.random() > 0.005) continue;
        ...
        console.error("Mahjong Way Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Royal Mahjong Way 5-Reel Slot Engine!"); });

// মাহজং গেম নিজস্ব কাস্টম ৫৪০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 5400; 
server.listen(PORT, () => { console.log(`🎡 Royal Mahjong Way Engine Running on port ${PORT}`); });
